...
func TestForwardRepositoryPersistsPerIPLimits(t *testing.T) {
	...
	now := time.Now().UnixMilli()
	forwardID, err := r.CreateForwardTx(1, "admin", "per-ip-forward", 2, "1.1.1.1:443", "fifo", "failover", now, 1, []int64{3}, 24000, "", nil, 0, 5, int64(21), 0, "gost")
	if err != nil {
	...
	if err := r.UpdateForward(forwardID, "per-ip-forward", 2, "2.2.2.2:443", "fifo", "failover", now+1, nil, 0, 9, int64(22), 0, "gost"); err != nil {
...
func TestRollbackForwardFieldsRestoresPerIPLimits(t *testing.T) {
	...
	now := time.Now().UnixMilli()
	forwardID, err := r.CreateForwardTx(1, "admin", "rollback-per-ip-forward", 2, "1.1.1.1:443", "fifo", "failover", now, 1, nil, 0, "", nil, 7, 5, int64(21), 2, "gost")
	if err != nil {
		t.Fatalf("CreateForwardTx: %v", err)
	}
	if err := r.UpdateForward(forwardID, "rollback-per-ip-forward", 2, "2.2.2.2:443", "fifo", "failover", now+1, nil, 0, 0, nil, 0, "gost"); err != nil {
		t.Fatalf("UpdateForward: %v", err)
	}
